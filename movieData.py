...
def showFilmPage(id):
    ...
    movie = database.selectMovie(id) #ID --> terem szám
    top = tk.Toplevel()
    ...
    #Title
    title_lab = ttk.Label(top, text= movie.title, font=('helvetica', 24))
    title_lab.grid(column=0, row=1, columnspan=2, ipady=10)

    #Date
    date_lab1 = ttk.Label(top, text="Megjelenés éve: ", font=('helvetica', 12))
    date_lab1.grid(column=0, row=2 , ipady=5)
    date_lab2 = ttk.Label(top, text= movie.date, font=('helvetica', 12))
    date_lab2.grid(column=1, row=2 , ipady=5)

    #Genre
    genre_lab1 = ttk.Label(top, text="Műfaj: ", font=('helvetica', 12))
    genre_lab1.grid(column=0, row=3 , ipady=5)
    genre_lab2 = ttk.Label(top,text= movie.genre, font=('helvetica', 12))
    genre_lab2.grid(column=1, row=3 , ipady=5)

    #Playtime
    time_lab1 = ttk.Label(top, text="Játékidő: ", font=('helvetica', 12))
    time_lab1.grid(column=0, row=4 , ipady=5)
    time_lab2 = ttk.Label(top,text= movie.playtime, font=('helvetica', 12))
    time_lab2.grid(column=1, row=4 , ipady=5)

    #Foglalás
    btn = ttk.Button(top, bootstyle="info", text="Jegy foglalás")
